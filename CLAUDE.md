...
單一 `@media (max-width:600px)` 區塊（style 區段末尾，跟 print media 分開），主要處理觸控目標與版面均衡，不影響桌機版樣式：

- 出貨模式切換：`.ms-btns{width:100%}` + `.ms-btn{flex:1}` 讓兩顆按鈕平分寬度、`min-height:46px` 確保文字不論換不換行高度都一致（修正原本 FBA 標籤較長會換兩行、FBM 只有一行造成兩顆按鈕高度不一致的問題）。
